    ...
    F: FnMut(&str)->*const std::os::raw::c_void,
{
    ...
    unsafe{
        let data = CStr::from_ptr(gl.GetString(gl::EXTENSIONS) as *const _).to_bytes().to_vec();
        println!("{:?}",gl.CreateShader);
        gl.Clear(gl::COLOR_BUFFER_BIT);
        let  pg = gl.CreateProgram();
        gl.DeleteProgram(pg);
        // println!("shader create {:?}",shader);
